...
func NewGreeterApiInterface(impl GreeterProvider) *GreeterApiInterface {
	...
	gIface.methodMetadataMap["informaly_greet"] = gIface.InformalyGreetMetadata(GreeterInformalyGreetOutputType())
	gIface.methodMetadataMap["formaly_greet"] = gIface.FormalyGreetMetadata(GreeterFormalyGreetOutputType())
	return &gIface
}

func (gIface *GreeterApiInterface) informalyGreet(ctx *vapiCore_.ExecutionContext, methodId vapiCore_.MethodIdentifier, inputDataValue vapiData_.DataValue) vapiCore_.MethodResult {
	...
	opMetadata := gIface.methodMetadataMap["informaly_greet"]

	result, err := gIface.impl.InformalyGreet(ctx)
...
func (gIface *GreeterApiInterface) formalyGreet(ctx *vapiCore_.ExecutionContext, methodId vapiCore_.MethodIdentifier, inputDataValue vapiData_.DataValue) vapiCore_.MethodResult {
	acceptOptions := &vapiCore_.BindingAcceptOptions{
		MethodName: methodId.Name(),
		IsStream:   false,
		IsTaskOnly: false,
	...
	typeConverter := vapiBindings_.NewTypeConverter()
	opMetadata := gIface.methodMetadataMap["formaly_greet"]

	result, err := gIface.impl.FormalyGreet(ctx)
...
func (gIface *GreeterApiInterface) InformalyGreetMetadata(outputType vapiBindings_.BindingType) vapiProtocol_.OperationMetadata {
	methodDefinition := gIface.InformalyGreetMethodDefinition(outputType)
	...
	return vapiProtocol_.NewOperationMetadata(
		methodDefinition,
		greeterInformalyGreetInputType(),
		outputType,
		errorBindingMap,
		greeterInformalyGreetRestMetadata())
}
func (gIface *GreeterApiInterface) FormalyGreetMetadata(outputType vapiBindings_.BindingType) vapiProtocol_.OperationMetadata {
	methodDefinition := gIface.FormalyGreetMethodDefinition(outputType)
	...
	return vapiProtocol_.NewOperationMetadata(
		methodDefinition,
		greeterFormalyGreetInputType(),
		outputType,
		errorBindingMap,
		greeterFormalyGreetRestMetadata())
}

func (gIface *GreeterApiInterface) InformalyGreetMethodDefinition(outputType vapiBindings_.BindingType) *vapiCore_.MethodDefinition {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier(gIface.interfaceName)
	typeConverter := vapiBindings_.NewTypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(greeterInformalyGreetInputType())
	if inputError != nil {
		vapiLog_.Errorf("Error in ConvertToDataDefinition for GreeterApiInterface.informalyGreet method's input - %s",
	...
	if outputError != nil {
		vapiLog_.Errorf("Error in ConvertToDataDefinition for GreeterApiInterface.informalyGreet method's output - %s",
			vapiBindings_.VAPIerrorsToError(outputError).Error())
		return nil
	}

	methodIdentifier := vapiCore_.NewMethodIdentifier(interfaceIdentifier, "informaly_greet")
	...
	if errError1 != nil {
		vapiLog_.Errorf("Error in ConvertToDataDefinition for GreeterApiInterface.informalyGreet method's vapiStdErrors_.Unauthenticated error - %s",
...
func (gIface *GreeterApiInterface) FormalyGreetMethodDefinition(outputType vapiBindings_.BindingType) *vapiCore_.MethodDefinition {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier(gIface.interfaceName)
	typeConverter := vapiBindings_.NewTypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(greeterFormalyGreetInputType())
	if inputError != nil {
		vapiLog_.Errorf("Error in ConvertToDataDefinition for GreeterApiInterface.formalyGreet method's input - %s",
			vapiBindings_.VAPIerrorsToError(inputError).Error())
		return nil
	}
	...
	output, outputError := typeConverter.ConvertToDataDefinition(outputType)
	if outputError != nil {
		vapiLog_.Errorf("Error in ConvertToDataDefinition for GreeterApiInterface.formalyGreet method's output - %s",
			vapiBindings_.VAPIerrorsToError(outputError).Error())
		return nil
	}

	methodIdentifier := vapiCore_.NewMethodIdentifier(interfaceIdentifier, "formaly_greet")
	errorDefinitions := make([]vapiData_.ErrorDefinition, 0)
	errDef1, errError1 := typeConverter.ConvertToDataDefinition(vapiStdErrors_.UnauthenticatedBindingType())
	if errError1 != nil {
		vapiLog_.Errorf("Error in ConvertToDataDefinition for GreeterApiInterface.formalyGreet method's vapiStdErrors_.Unauthenticated error - %s",
			vapiBindings_.VAPIerrorsToError(errError1).Error())
		return nil
	}
	errorDefinitions = append(errorDefinitions, errDef1.(vapiData_.ErrorDefinition))
...
func (gIface *GreeterApiInterface) Invoke(ctx *vapiCore_.ExecutionContext, methodId vapiCore_.MethodIdentifier, inputDataValue vapiData_.DataValue) vapiCore_.MethodResult {
	...
	case "informaly_greet":
		return gIface.informalyGreet(ctx, methodId, inputDataValue)
	case "formaly_greet":
		return gIface.formalyGreet(ctx, methodId, inputDataValue)
	}
