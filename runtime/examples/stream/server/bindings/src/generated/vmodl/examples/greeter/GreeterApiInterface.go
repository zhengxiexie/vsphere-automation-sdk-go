...
func (gIface *GreeterApiInterface) greet(ctx *vapiCore_.ExecutionContext, methodId vapiCore_.MethodIdentifier, inputDataValue vapiData_.DataValue) vapiCore_.MethodResult {
	...
	goNativeResultChan, errorChan := gIface.impl.Greet(ctx)

	cancelCtx, cancelFunc := context.WithCancel(ctx.Context())
	ctx.WithContext(cancelCtx)
	methodResultChan := make(chan vapiCore_.MonoResult)
	methodResult := vapiCore_.NewStreamMethodResult(methodResultChan, cancelFunc)
	go func() {

		isResultChanDone := make(chan bool)

		var wg sync.WaitGroup
		wg.Add(1)
		go func() {
			defer wg.Done()
			for resultElement := range goNativeResultChan {
				output, errorInOutput := typeConverter.ConvertToVapi(resultElement, opMetadata.OutputType())
				if errorInOutput != nil {
					methodResultChan <- createGreeterErrorResult(errorInOutput, "Invalid output")
					return
				}
				methodResultChan <- vapiCore_.NewMonoResult(output, nil)
			}

			isResultChanDone <- true
		...
		go func() {
			defer wg.Done()
			var err error = nil
			for errElement := range errorChan {
				err = errElement
			}
			<-isResultChanDone
			if err != nil {
				...
				if !exist {
					methodResultChan <- createGreeterErrorResult([]error{err}, "Invalid error returned by server")
					return
				}
				errorValue, errorInError := typeConverter.ConvertToVapi(err, errType)
				if errorInError != nil {
					methodResultChan <- createGreeterErrorResult(errorInError, "Invalid error")
					return
				}
				methodResultChan <- vapiCore_.NewMonoResult(nil, errorValue.(*vapiData_.ErrorValue))
			}
		}()
...
func (gIface *GreeterApiInterface) GreetMetadata(outputType vapiBindings_.BindingType) vapiProtocol_.OperationMetadata {
	...
	errorBindingMap[vapiStdErrors_.InternalServerError{}.Error()] = vapiStdErrors_.InternalServerErrorBindingType()
...
func (gIface *GreeterApiInterface) GreetMethodDefinition(outputType vapiBindings_.BindingType) *vapiCore_.MethodDefinition {
	...
	errDef1, errError1 := typeConverter.ConvertToDataDefinition(vapiStdErrors_.InternalServerErrorBindingType())
	if errError1 != nil {
		vapiLog_.Errorf("Error in ConvertToDataDefinition for GreeterApiInterface.greet method's vapiStdErrors_.InternalServerError error - %s",
