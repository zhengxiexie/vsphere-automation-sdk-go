...
type FlowDetailsClient interface {
	...
	Get(sitePathParam *string) (nsx_policyModel.MonitoringInfo, error)
}
...
func NewFlowDetailsClient(connector vapiProtocolClient_.Connector) *flowDetailsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.observability.flow_details")
...
func (fIface *flowDetailsClient) Get(sitePathParam *string) (nsx_policyModel.MonitoringInfo, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.MonitoringInfo
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.MonitoringInfo), nil
	} else {
