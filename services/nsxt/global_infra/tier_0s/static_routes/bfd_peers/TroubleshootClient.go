...
func NewTroubleshootClient(connector vapiProtocolClient_.Connector) *troubleshootClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.tier_0s.static_routes.bfd_peers.troubleshoot")
...
func (tIface *troubleshootClient) Get(tier0IdParam string) (nsx_policyModel.StaticRouteBfdTroubleshootConfig, error) {
	...
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.static_routes.bfd_peers.troubleshoot", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.StaticRouteBfdTroubleshootConfig
...
func (tIface *troubleshootClient) Patch(tier0IdParam string, staticRouteBfdTroubleshootConfigParam nsx_policyModel.StaticRouteBfdTroubleshootConfig, overrideParam *bool) error {
	...
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.static_routes.bfd_peers.troubleshoot", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), tIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (tIface *troubleshootClient) Update(tier0IdParam string, staticRouteBfdTroubleshootConfigParam nsx_policyModel.StaticRouteBfdTroubleshootConfig, overrideParam *bool) (nsx_policyModel.StaticRouteBfdTroubleshootConfig, error) {
	...
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.static_routes.bfd_peers.troubleshoot", "update", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.StaticRouteBfdTroubleshootConfig
	if methodResult.IsSuccess() {
