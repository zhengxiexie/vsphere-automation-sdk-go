...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.enforcement_points.edge_clusters.remote_tunnel_connectivity.status")
...
func (sIface *statusClient) Get(siteIdParam string, enforcementpointIdParam string, edgeClusterIdParam string) (nsx_policyModel.PolicyEdgeClusterRemoteTunnelConnectivityStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.PolicyEdgeClusterRemoteTunnelConnectivityStatus
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.enforcement_points.edge_clusters.remote_tunnel_connectivity.status", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.PolicyEdgeClusterRemoteTunnelConnectivityStatus
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.PolicyEdgeClusterRemoteTunnelConnectivityStatus), nil
	} else {
