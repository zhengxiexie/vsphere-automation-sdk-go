...
type Listener_certificateClient interface {
	...
	Get(addressParam string, portParam int64) (nsx_policyModel.TlsListenerCertificate, error)
}
...
func NewListener_certificateClient(connector vapiProtocolClient_.Connector) *listener_certificateClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.listener_certificate")
...
func (lIface *listener_certificateClient) Get(addressParam string, portParam int64) (nsx_policyModel.TlsListenerCertificate, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.TlsListenerCertificate
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.TlsListenerCertificate), nil
	} else {
