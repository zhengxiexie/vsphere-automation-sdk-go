...
type VmTagReplicationPoliciesClient interface {
	...
	Get(idParam string) (nsx_policyModel.VMTagReplicationPolicy, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List(cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.VMTagReplicationPolicyListResult, error)
}
...
func NewVmTagReplicationPoliciesClient(connector vapiProtocolClient_.Connector) *vmTagReplicationPoliciesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.vm_tag_replication_policies")
...
func (vIface *vmTagReplicationPoliciesClient) Get(idParam string) (nsx_policyModel.VMTagReplicationPolicy, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.VMTagReplicationPolicy
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.VMTagReplicationPolicy), nil
	} else {
...
func (vIface *vmTagReplicationPoliciesClient) List(cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.VMTagReplicationPolicyListResult, error) {
	typeConverter := vIface.connector.TypeConverter()
	executionContext := vIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsx_policyModel.VMTagReplicationPolicyListResult
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.VMTagReplicationPolicyListResult), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), vIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
			return emptyOutput, vapiBindings_.VAPIerrorsToError(errorInError)
