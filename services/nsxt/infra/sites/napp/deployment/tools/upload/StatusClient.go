...
type StatusClient interface {
	...
	Get(siteIdParam string, bundleIdParam string) (nsx_policyModel.UploadBundleStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sites.napp.deployment.tools.upload.status")
...
func (sIface *statusClient) Get(siteIdParam string, bundleIdParam string) (nsx_policyModel.UploadBundleStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.UploadBundleStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.UploadBundleStatus), nil
	} else {
