...
type FormFactorsClient interface {
	...
	Get(siteIdParam string) (nsx_policyModel.PlatformFormFactors, error)
}
...
func NewFormFactorsClient(connector vapiProtocolClient_.Connector) *formFactorsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sites.napp.deployment.platform.form_factors")
...
func (fIface *formFactorsClient) Get(siteIdParam string) (nsx_policyModel.PlatformFormFactors, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.PlatformFormFactors
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.PlatformFormFactors), nil
	} else {
