...
type ResetClient interface {
	...
	Update(siteIdParam string) (nsx_policyModel.DeploymentRegistry, error)
}
...
func NewResetClient(connector vapiProtocolClient_.Connector) *resetClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sites.napp.deployment.registry.reset")
...
func (rIface *resetClient) Update(siteIdParam string) (nsx_policyModel.DeploymentRegistry, error) {
	...
	sv := vapiBindings_.NewStructValueBuilder(resetUpdateInputType(), typeConverter)
