...
type StatusClient interface {
	...
	Get(siteIdParam string) (nsx_policyModel.DeploymentProgressStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sites.napp.deployment.upgrade_coordinator.status")
...
func (sIface *statusClient) Get(siteIdParam string) (nsx_policyModel.DeploymentProgressStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.DeploymentProgressStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.DeploymentProgressStatus), nil
	} else {
