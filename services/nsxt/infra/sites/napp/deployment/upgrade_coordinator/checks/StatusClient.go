...
type StatusClient interface {
	...
	Get(siteIdParam string) (nsx_policyModel.DeploymentChecksStatusResult, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sites.napp.deployment.upgrade_coordinator.checks.status")
...
func (sIface *statusClient) Get(siteIdParam string) (nsx_policyModel.DeploymentChecksStatusResult, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.DeploymentChecksStatusResult
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.DeploymentChecksStatusResult), nil
	} else {
