...
func NewMemberGroupsClient(connector vapiProtocolClient_.Connector) *memberGroupsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.firewall_identity_stores.groups.member_groups")
...
func (mIface *memberGroupsClient) List(firewallIdentityStoreIdParam string, groupIdParam string, cursorParam *string, enforcementPointPathParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.DirectoryGroupMemberListResults, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.DirectoryGroupMemberListResults
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.DirectoryGroupMemberListResults), nil
	} else {
