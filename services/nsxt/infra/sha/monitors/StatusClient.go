...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sha.monitors.status")
...
func (sIface *statusClient) Get(monitorIdParam string, transportNodeIdParam string) (nsx_policyModel.ShaMonitorStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.sha.monitors.status", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.ShaMonitorStatus
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.ShaMonitorStatus), nil
	} else {
