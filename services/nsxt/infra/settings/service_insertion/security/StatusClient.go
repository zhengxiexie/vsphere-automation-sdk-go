...
type StatusClient interface {
	...
	Get() (nsx_policyModel.PolicySIStatusConfiguration, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Patch(policySIStatusConfigurationParam nsx_policyModel.PolicySIStatusConfiguration) error
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(policySIStatusConfigurationParam nsx_policyModel.PolicySIStatusConfiguration) (nsx_policyModel.PolicySIStatusConfiguration, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.settings.service_insertion.security.status")
...
func (sIface *statusClient) Get() (nsx_policyModel.PolicySIStatusConfiguration, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.PolicySIStatusConfiguration
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.PolicySIStatusConfiguration), nil
	} else {
...
func (sIface *statusClient) Patch(policySIStatusConfigurationParam nsx_policyModel.PolicySIStatusConfiguration) error {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	...
	sv := vapiBindings_.NewStructValueBuilder(statusPatchInputType(), typeConverter)
...
func (sIface *statusClient) Update(policySIStatusConfigurationParam nsx_policyModel.PolicySIStatusConfiguration) (nsx_policyModel.PolicySIStatusConfiguration, error) {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
