...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.settings.firewall.idfw.compute_collections.transport_nodes.status")
...
func (sIface *statusClient) List(computeCollectionIdParam string, enforcementPointPathParam *string) (nsx_policyModel.IdfwTransportNodeStatusListResult, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.IdfwTransportNodeStatusListResult
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.IdfwTransportNodeStatusListResult), nil
	} else {
