...
type StatusClient interface {
	...
	Get(computeCollectionIdParam string, enforcementPointPathParam *string) (nsx_policyModel.IdfwComputeCollectionStatus, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List(enforcementPointPathParam *string) (nsx_policyModel.IdfwComputeCollectionListResult, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.settings.firewall.idfw.compute_collections.status")
...
func (sIface *statusClient) Get(computeCollectionIdParam string, enforcementPointPathParam *string) (nsx_policyModel.IdfwComputeCollectionStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.IdfwComputeCollectionStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.IdfwComputeCollectionStatus), nil
	} else {
...
func (sIface *statusClient) List(enforcementPointPathParam *string) (nsx_policyModel.IdfwComputeCollectionListResult, error) {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsx_policyModel.IdfwComputeCollectionListResult
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.IdfwComputeCollectionListResult), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
			return emptyOutput, vapiBindings_.VAPIerrorsToError(errorInError)
