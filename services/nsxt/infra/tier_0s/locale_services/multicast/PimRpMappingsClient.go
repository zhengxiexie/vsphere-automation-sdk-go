...
type PimRpMappingsClient interface {
	...
	List(tier0IdParam string, localeServicesIdParam string, cursorParam *string, edgePathParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.PimRpMappings, error)
}
...
func NewPimRpMappingsClient(connector vapiProtocolClient_.Connector) *pimRpMappingsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.tier_0s.locale_services.multicast.pim_rp_mappings")
...
func (pIface *pimRpMappingsClient) GetErrorBindingType(errorName string) vapiBindings_.BindingType {
...
func (pIface *pimRpMappingsClient) List(tier0IdParam string, localeServicesIdParam string, cursorParam *string, edgePathParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.PimRpMappings, error) {
	...
	sv := vapiBindings_.NewStructValueBuilder(pimRpMappingsListInputType(), typeConverter)
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.PimRpMappings
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.PimRpMappings), nil
	} else {
