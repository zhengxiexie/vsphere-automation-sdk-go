...
type KubernetesGatewaysClient interface {
	...
	List(domainIdParam string, groupIdParam string, clusterIdParam *string, cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.PolicyGroupKubernetesGatewayListResult, error)
}
...
func NewKubernetesGatewaysClient(connector vapiProtocolClient_.Connector) *kubernetesGatewaysClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.domains.groups.members.kubernetes_gateways")
...
func (kIface *kubernetesGatewaysClient) GetErrorBindingType(errorName string) vapiBindings_.BindingType {
...
func (kIface *kubernetesGatewaysClient) List(domainIdParam string, groupIdParam string, clusterIdParam *string, cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.PolicyGroupKubernetesGatewayListResult, error) {
	...
	sv := vapiBindings_.NewStructValueBuilder(kubernetesGatewaysListInputType(), typeConverter)
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.PolicyGroupKubernetesGatewayListResult
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.PolicyGroupKubernetesGatewayListResult), nil
	} else {
