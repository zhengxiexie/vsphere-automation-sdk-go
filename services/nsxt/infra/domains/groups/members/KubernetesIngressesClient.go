...
type KubernetesIngressesClient interface {
	...
	List(domainIdParam string, groupIdParam string, clusterIdParam *string, cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.PolicyGroupKubernetesIngressListResult, error)
}
...
func NewKubernetesIngressesClient(connector vapiProtocolClient_.Connector) *kubernetesIngressesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.domains.groups.members.kubernetes_ingresses")
...
func (kIface *kubernetesIngressesClient) GetErrorBindingType(errorName string) vapiBindings_.BindingType {
...
func (kIface *kubernetesIngressesClient) List(domainIdParam string, groupIdParam string, clusterIdParam *string, cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.PolicyGroupKubernetesIngressListResult, error) {
	...
	sv := vapiBindings_.NewStructValueBuilder(kubernetesIngressesListInputType(), typeConverter)
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.PolicyGroupKubernetesIngressListResult
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.PolicyGroupKubernetesIngressListResult), nil
	} else {
