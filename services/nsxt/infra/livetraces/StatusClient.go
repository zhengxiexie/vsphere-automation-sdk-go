...
type StatusClient interface {
	...
	Get(livetraceIdParam string, enforcementPointPathParam *string) (nsx_policyModel.LiveTraceStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.livetraces.status")
...
func (sIface *statusClient) Get(livetraceIdParam string, enforcementPointPathParam *string) (nsx_policyModel.LiveTraceStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.LiveTraceStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.LiveTraceStatus), nil
	} else {
