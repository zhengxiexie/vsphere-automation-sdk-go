...
type DetailedStatusClient interface {
	...
	Get(orgIdParam string, projectIdParam string, lbServiceIdParam string, lbPoolIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateLBPoolStatus, error)
}
...
func NewDetailedStatusClient(connector vapiProtocolClient_.Connector) *detailedStatusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.infra.lb_services.lb_pools.detailed_status")
...
func (dIface *detailedStatusClient) Get(orgIdParam string, projectIdParam string, lbServiceIdParam string, lbPoolIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateLBPoolStatus, error) {
	...
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.lb_services.lb_pools.detailed_status", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.AggregateLBPoolStatus
