...
type DetailedStatusClient interface {
	...
	Get(orgIdParam string, projectIdParam string, tier1IdParam string, serviceIdParam string, sessionIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateL2VPNSessionStatus, error)
}
...
func NewDetailedStatusClient(connector vapiProtocolClient_.Connector) *detailedStatusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.infra.tier_1s.l2vpn_services.sessions.detailed_status")
...
func (dIface *detailedStatusClient) Get(orgIdParam string, projectIdParam string, tier1IdParam string, serviceIdParam string, sessionIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateL2VPNSessionStatus, error) {
	...
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.tier_1s.l2vpn_services.sessions.detailed_status", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.AggregateL2VPNSessionStatus
