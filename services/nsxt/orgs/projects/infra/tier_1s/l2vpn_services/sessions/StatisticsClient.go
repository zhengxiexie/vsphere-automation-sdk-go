...
type StatisticsClient interface {
	...
	Get(orgIdParam string, projectIdParam string, tier1IdParam string, serviceIdParam string, sessionIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateL2VPNSessionStatistics, error)
}
...
func NewStatisticsClient(connector vapiProtocolClient_.Connector) *statisticsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.infra.tier_1s.l2vpn_services.sessions.statistics")
...
func (sIface *statisticsClient) Get(orgIdParam string, projectIdParam string, tier1IdParam string, serviceIdParam string, sessionIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_policyModel.AggregateL2VPNSessionStatistics, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.AggregateL2VPNSessionStatistics
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.tier_1s.l2vpn_services.sessions.statistics", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.AggregateL2VPNSessionStatistics
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.AggregateL2VPNSessionStatistics), nil
	} else {
