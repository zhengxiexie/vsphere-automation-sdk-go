...
type RemoteMacClient interface {
	...
	Get(orgIdParam string, projectIdParam string, tier1IdParam string, serviceIdParam string, sessionIdParam string, enforcementPointPathParam *string, segmentPathParam *string) (nsx_policyModel.AggregateL2VpnSessionRemoteMac, error)
}
...
func NewRemoteMacClient(connector vapiProtocolClient_.Connector) *remoteMacClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.infra.tier_1s.l2vpn_services.sessions.remote_mac")
...
func (rIface *remoteMacClient) Get(orgIdParam string, projectIdParam string, tier1IdParam string, serviceIdParam string, sessionIdParam string, enforcementPointPathParam *string, segmentPathParam *string) (nsx_policyModel.AggregateL2VpnSessionRemoteMac, error) {
	...
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.tier_1s.l2vpn_services.sessions.remote_mac", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.AggregateL2VpnSessionRemoteMac
