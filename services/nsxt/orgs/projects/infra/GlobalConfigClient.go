...
func NewGlobalConfigClient(connector vapiProtocolClient_.Connector) *globalConfigClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.infra.global_config")
...
func (gIface *globalConfigClient) Get(orgIdParam string, projectIdParam string, applyLocallyParam *bool) (nsx_policyModel.GlobalConfig, error) {
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.global_config", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.GlobalConfig
...
func (gIface *globalConfigClient) Patch(orgIdParam string, projectIdParam string, globalConfigParam nsx_policyModel.GlobalConfig, applyLocallyParam *bool) error {
	typeConverter := gIface.connector.TypeConverter()
	executionContext := gIface.connector.NewExecutionContext()
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.global_config", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), gIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (gIface *globalConfigClient) Update(orgIdParam string, projectIdParam string, globalConfigParam nsx_policyModel.GlobalConfig, applyLocallyParam *bool) (nsx_policyModel.GlobalConfig, error) {
	typeConverter := gIface.connector.TypeConverter()
	executionContext := gIface.connector.NewExecutionContext()
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.global_config", "update", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.GlobalConfig
	if methodResult.IsSuccess() {
