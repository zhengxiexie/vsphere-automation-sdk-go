...
type ThresholdClient interface {
	...
	Get(orgIdParam string, projectIdParam string) (nsx_policyModel.CapacityThresholdList, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Patch(orgIdParam string, projectIdParam string, capacityThresholdParam nsx_policyModel.CapacityThreshold) error
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(orgIdParam string, projectIdParam string, capacityThresholdListParam nsx_policyModel.CapacityThresholdList) (nsx_policyModel.CapacityThresholdList, error)
}
...
func NewThresholdClient(connector vapiProtocolClient_.Connector) *thresholdClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.infra.capacity.threshold")
...
func (tIface *thresholdClient) Get(orgIdParam string, projectIdParam string) (nsx_policyModel.CapacityThresholdList, error) {
	...
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.capacity.threshold", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.CapacityThresholdList
...
func (tIface *thresholdClient) Patch(orgIdParam string, projectIdParam string, capacityThresholdParam nsx_policyModel.CapacityThreshold) error {
	typeConverter := tIface.connector.TypeConverter()
	executionContext := tIface.connector.NewExecutionContext()
	...
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.capacity.threshold", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), tIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (tIface *thresholdClient) Update(orgIdParam string, projectIdParam string, capacityThresholdListParam nsx_policyModel.CapacityThresholdList) (nsx_policyModel.CapacityThresholdList, error) {
	typeConverter := tIface.connector.TypeConverter()
	executionContext := tIface.connector.NewExecutionContext()
	...
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.capacity.threshold", "update", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.CapacityThresholdList
	if methodResult.IsSuccess() {
