...
type VcFolderStatusClient interface {
	...
	Get(orgIdParam string, projectIdParam string, vpcIdParam string) (nsx_policyModel.VCFolderStatus, error)
}
...
func NewVcFolderStatusClient(connector vapiProtocolClient_.Connector) *vcFolderStatusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.vpcs.vc_folder_status")
...
func (vIface *vcFolderStatusClient) Get(orgIdParam string, projectIdParam string, vpcIdParam string) (nsx_policyModel.VCFolderStatus, error) {
	...
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.vpcs.vc_folder_status", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.VCFolderStatus
