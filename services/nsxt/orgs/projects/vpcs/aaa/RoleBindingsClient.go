...
type RoleBindingsClient interface {
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Patch(orgIdParam string, projectIdParam string, vpcIdParam string, roleBindingParam nsx_policyModel.RoleBinding) (nsx_policyModel.RoleBinding, error)
}
...
func (rIface *roleBindingsClient) Patch(orgIdParam string, projectIdParam string, vpcIdParam string, roleBindingParam nsx_policyModel.RoleBinding) (nsx_policyModel.RoleBinding, error) {
	typeConverter := rIface.connector.TypeConverter()
	executionContext := rIface.connector.NewExecutionContext()
	...
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.vpcs.aaa.role_bindings", "patch", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.RoleBinding
	if methodResult.IsSuccess() {
