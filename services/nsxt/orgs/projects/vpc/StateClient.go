...
func NewStateClient(connector vapiProtocolClient_.Connector) *stateClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.vpc.state")
...
func (sIface *stateClient) Get(orgIdParam string, projectIdParam string, vpcIdParam string) (nsx_policyModel.VpcState, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.vpc.state", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.VpcState
