...
func NewLoginLogoutEventsClient(connector vapiProtocolClient_.Connector) *loginLogoutEventsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.system.input.login_logout_events")
...
func (lIface *loginLogoutEventsClient) GetErrorBindingType(errorName string) vapiBindings_.BindingType {
...
func (lIface *loginLogoutEventsClient) Create(loginLogoutEventCollectionParam nsx_policyModel.LoginLogoutEventCollection) error {
	...
	sv := vapiBindings_.NewStructValueBuilder(loginLogoutEventsCreateInputType(), typeConverter)
