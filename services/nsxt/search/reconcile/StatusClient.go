...
type StatusClient interface {
	...
	Get(actionParam *string) (nsx_policyModel.SearchReconcileStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.search.reconcile.status")
...
func (sIface *statusClient) Get(actionParam *string) (nsx_policyModel.SearchReconcileStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.SearchReconcileStatus
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.search.reconcile.status", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.SearchReconcileStatus
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.SearchReconcileStatus), nil
	} else {
