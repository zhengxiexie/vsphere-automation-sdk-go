...
func NewWatermarkClient(connector vapiProtocolClient_.Connector) *watermarkClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.watermark")
...
func (wIface *watermarkClient) Create(nSXManagerDeploymentInfoParam nsx_policyModel.NSXManagerDeploymentInfo) (nsx_policyModel.NSXManagerDeploymentInfo, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.NSXManagerDeploymentInfo
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := wIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.watermark", "create", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.NSXManagerDeploymentInfo
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.NSXManagerDeploymentInfo), nil
	} else {
...
func (wIface *watermarkClient) Delete() error {
	...
	methodResult := wIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.watermark", "delete", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), wIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (wIface *watermarkClient) Get() (nsx_policyModel.NSXManagerDeploymentInfo, error) {
	typeConverter := wIface.connector.TypeConverter()
	executionContext := wIface.connector.NewExecutionContext()
	...
	methodResult := wIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.watermark", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.NSXManagerDeploymentInfo
	if methodResult.IsSuccess() {
...
func (wIface *watermarkClient) Update(nSXManagerDeploymentInfoParam nsx_policyModel.NSXManagerDeploymentInfo) (nsx_policyModel.NSXManagerDeploymentInfo, error) {
	typeConverter := wIface.connector.TypeConverter()
	executionContext := wIface.connector.NewExecutionContext()
	...
	methodResult := wIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.watermark", "update", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.NSXManagerDeploymentInfo
	if methodResult.IsSuccess() {
