...
type StatusClient interface {
	...
	Get() (nsx_policyModel.CurrentBackupOperationStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.cluster.backups.status")
...
func (sIface *statusClient) Get() (nsx_policyModel.CurrentBackupOperationStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.CurrentBackupOperationStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.CurrentBackupOperationStatus), nil
	} else {
