...
func NewRetry_with_updated_configClient(connector vapiProtocolClient_.Connector) *retry_with_updated_configClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.cluster.restore.action.retry_with_updated_config")
...
func (rIface *retry_with_updated_configClient) Create() (nsx_policyModel.ClusterRestoreStatus, error) {
	...
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.cluster.restore.action.retry_with_updated_config", "create", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.ClusterRestoreStatus
