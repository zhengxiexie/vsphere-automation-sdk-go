...
type StatusClient interface {
	...
	Get(nodeIdParam string) (nsx_policyModel.ALBControllerNodeVMDeploymentStatusReport, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.alb.controller_nodes.deployments.status")
...
func (sIface *statusClient) Get(nodeIdParam string) (nsx_policyModel.ALBControllerNodeVMDeploymentStatusReport, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.ALBControllerNodeVMDeploymentStatusReport
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.ALBControllerNodeVMDeploymentStatusReport), nil
	} else {
