...
type VersionWhitelistClient interface {
	...
	Get(componentTypeParam string) (nsx_global_policyModel.AcceptableComponentVersion, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List() (nsx_global_policyModel.AcceptableComponentVersionList, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(componentTypeParam string, versionListParam nsx_global_policyModel.VersionList) error
}
...
func NewVersionWhitelistClient(connector vapiProtocolClient_.Connector) *versionWhitelistClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.upgrade.version_whitelist")
...
func (vIface *versionWhitelistClient) Get(componentTypeParam string) (nsx_global_policyModel.AcceptableComponentVersion, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.AcceptableComponentVersion
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.AcceptableComponentVersion), nil
	} else {
...
func (vIface *versionWhitelistClient) List() (nsx_global_policyModel.AcceptableComponentVersionList, error) {
	typeConverter := vIface.connector.TypeConverter()
	executionContext := vIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.AcceptableComponentVersionList
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.AcceptableComponentVersionList), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), vIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
			return emptyOutput, vapiBindings_.VAPIerrorsToError(errorInError)
...
func (vIface *versionWhitelistClient) Update(componentTypeParam string, versionListParam nsx_global_policyModel.VersionList) error {
	typeConverter := vIface.connector.TypeConverter()
	executionContext := vIface.connector.NewExecutionContext()
