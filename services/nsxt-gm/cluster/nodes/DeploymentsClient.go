...
type DeploymentsClient interface {
	...
	Create(addClusterNodeVMInfoParam nsx_global_policyModel.AddClusterNodeVMInfo) (nsx_global_policyModel.ClusterNodeVMDeploymentRequestList, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Get(nodeIdParam string) (nsx_global_policyModel.ClusterNodeVMDeploymentRequest, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List() (nsx_global_policyModel.ClusterNodeVMDeploymentRequestList, error)
}
...
func NewDeploymentsClient(connector vapiProtocolClient_.Connector) *deploymentsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.cluster.nodes.deployments")
...
func (dIface *deploymentsClient) Create(addClusterNodeVMInfoParam nsx_global_policyModel.AddClusterNodeVMInfo) (nsx_global_policyModel.ClusterNodeVMDeploymentRequestList, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.ClusterNodeVMDeploymentRequestList
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.ClusterNodeVMDeploymentRequestList), nil
	} else {
...
func (dIface *deploymentsClient) Get(nodeIdParam string) (nsx_global_policyModel.ClusterNodeVMDeploymentRequest, error) {
	typeConverter := dIface.connector.TypeConverter()
	executionContext := dIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.ClusterNodeVMDeploymentRequest
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.ClusterNodeVMDeploymentRequest), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), dIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
			return emptyOutput, vapiBindings_.VAPIerrorsToError(errorInError)
...
func (dIface *deploymentsClient) List() (nsx_global_policyModel.ClusterNodeVMDeploymentRequestList, error) {
	typeConverter := dIface.connector.TypeConverter()
	executionContext := dIface.connector.NewExecutionContext()
