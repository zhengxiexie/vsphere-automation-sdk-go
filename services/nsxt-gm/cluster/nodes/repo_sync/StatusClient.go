...
type StatusClient interface {
	...
	Get(nodeIdParam string) (nsx_global_policyModel.RepoSyncStatusReport, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.cluster.nodes.repo_sync.status")
...
func (sIface *statusClient) Get(nodeIdParam string) (nsx_global_policyModel.RepoSyncStatusReport, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.RepoSyncStatusReport
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.RepoSyncStatusReport), nil
	} else {
