...
type StatusClient interface {
	...
	Get(nodeIdParam string) (nsx_global_policyModel.ClusterNodeVMDeploymentStatusReport, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.cluster.nodes.deployments.status")
...
func (sIface *statusClient) Get(nodeIdParam string) (nsx_global_policyModel.ClusterNodeVMDeploymentStatusReport, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.ClusterNodeVMDeploymentStatusReport
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.ClusterNodeVMDeploymentStatusReport), nil
	} else {
