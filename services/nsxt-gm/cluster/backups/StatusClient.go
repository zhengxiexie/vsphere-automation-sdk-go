...
type StatusClient interface {
	...
	Get() (nsx_global_policyModel.CurrentBackupOperationStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.cluster.backups.status")
...
func (sIface *statusClient) Get() (nsx_global_policyModel.CurrentBackupOperationStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.CurrentBackupOperationStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.CurrentBackupOperationStatus), nil
	} else {
