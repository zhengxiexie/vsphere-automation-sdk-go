...
type StatusClient interface {
	...
	Get(metadataProxyIdParam string, enforcementPointPathParam *string, segmentPathParam *string, sourceParam *string) (nsx_global_policyModel.PolicyMetadataProxyStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.metadata_proxies.status")
...
func (sIface *statusClient) Get(metadataProxyIdParam string, enforcementPointPathParam *string, segmentPathParam *string, sourceParam *string) (nsx_global_policyModel.PolicyMetadataProxyStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.PolicyMetadataProxyStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.PolicyMetadataProxyStatus), nil
	} else {
