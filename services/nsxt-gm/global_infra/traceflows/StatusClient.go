...
type StatusClient interface {
	...
	Get(traceflowIdParam string, enforcementPointPathParam *string) (nsx_global_policyModel.Traceflow, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.traceflows.status")
...
func (sIface *statusClient) Get(traceflowIdParam string, enforcementPointPathParam *string) (nsx_global_policyModel.Traceflow, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.Traceflow
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.Traceflow), nil
	} else {
