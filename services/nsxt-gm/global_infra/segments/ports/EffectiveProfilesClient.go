...
type EffectiveProfilesClient interface {
	...
	Get(segmentIdParam string, portIdParam string, enforcementPointPathParam *string) (nsx_global_policyModel.EffectiveProfilesResponse, error)
}
...
func NewEffectiveProfilesClient(connector vapiProtocolClient_.Connector) *effectiveProfilesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.segments.ports.effective_profiles")
...
func (eIface *effectiveProfilesClient) Get(segmentIdParam string, portIdParam string, enforcementPointPathParam *string) (nsx_global_policyModel.EffectiveProfilesResponse, error) {
	...
	methodResult := eIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.segments.ports.effective_profiles", "get", inputDataValue, executionContext)
	var emptyOutput nsx_global_policyModel.EffectiveProfilesResponse
