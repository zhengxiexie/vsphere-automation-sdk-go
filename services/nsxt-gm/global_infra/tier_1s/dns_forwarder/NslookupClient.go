...
type NslookupClient interface {
	...
	Get(tier1IdParam string, addressParam *string, enforcementPointPathParam *string) (nsx_global_policyModel.AggregatePolicyDnsAnswer, error)
}
...
func NewNslookupClient(connector vapiProtocolClient_.Connector) *nslookupClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.tier_1s.dns_forwarder.nslookup")
...
func (nIface *nslookupClient) Get(tier1IdParam string, addressParam *string, enforcementPointPathParam *string) (nsx_global_policyModel.AggregatePolicyDnsAnswer, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.AggregatePolicyDnsAnswer
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.AggregatePolicyDnsAnswer), nil
	} else {
