...
type StatusClient interface {
	...
	Get(tier1IdParam string, enforcementPointPathParam *string) (nsx_global_policyModel.AggregateDNSForwarderStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.tier_1s.dns_forwarder.status")
...
func (sIface *statusClient) Get(tier1IdParam string, enforcementPointPathParam *string) (nsx_global_policyModel.AggregateDNSForwarderStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.AggregateDNSForwarderStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.AggregateDNSForwarderStatus), nil
	} else {
