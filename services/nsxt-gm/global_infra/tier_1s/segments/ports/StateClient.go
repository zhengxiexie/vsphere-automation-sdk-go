...
type StateClient interface {
	...
	Get(tier1IdParam string, segmentIdParam string, portIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_global_policyModel.SegmentPortState, error)
}
...
func NewStateClient(connector vapiProtocolClient_.Connector) *stateClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.tier_1s.segments.ports.state")
...
func (sIface *stateClient) Get(tier1IdParam string, segmentIdParam string, portIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_global_policyModel.SegmentPortState, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.SegmentPortState
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.SegmentPortState), nil
	} else {
