...
type NumberOfRoutesClient interface {
	...
	// @return com.vmware.nsx_global_policy.model.Tier0NumberOfRoutesResult
	//
	...
	Get(tier0IdParam string, edgePathParam string, enforcementPointPathParam *string, includeChildVrfParam *bool) (nsx_global_policyModel.Tier0NumberOfRoutesResult, error)
}
...
func NewNumberOfRoutesClient(connector vapiProtocolClient_.Connector) *numberOfRoutesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.tier_0s.number_of_routes")
...
func (nIface *numberOfRoutesClient) Get(tier0IdParam string, edgePathParam string, enforcementPointPathParam *string, includeChildVrfParam *bool) (nsx_global_policyModel.Tier0NumberOfRoutesResult, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.Tier0NumberOfRoutesResult
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.tier_0s.number_of_routes", "get", inputDataValue, executionContext)
	var emptyOutput nsx_global_policyModel.Tier0NumberOfRoutesResult
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.Tier0NumberOfRoutesResult), nil
	} else {
