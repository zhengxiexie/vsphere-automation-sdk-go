...
type StatusClient interface {
	...
	Get(operationIdParam string, cursorParam *string, enforcementPointPathParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_global_policyModel.TagBulkOperationStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.tags.tag_operations.status")
...
func (sIface *statusClient) Get(operationIdParam string, cursorParam *string, enforcementPointPathParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_global_policyModel.TagBulkOperationStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.TagBulkOperationStatus
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.tags.tag_operations.status", "get", inputDataValue, executionContext)
	var emptyOutput nsx_global_policyModel.TagBulkOperationStatus
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.TagBulkOperationStatus), nil
	} else {
