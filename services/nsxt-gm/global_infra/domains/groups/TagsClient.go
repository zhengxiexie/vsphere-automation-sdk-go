...
type TagsClient interface {
	...
	Get(domainIdParam string, groupIdParam string) (nsx_global_policyModel.GroupTagsList, error)
}
...
func NewTagsClient(connector vapiProtocolClient_.Connector) *tagsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.domains.groups.tags")
...
func (tIface *tagsClient) Get(domainIdParam string, groupIdParam string) (nsx_global_policyModel.GroupTagsList, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.GroupTagsList
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.GroupTagsList), nil
	} else {
