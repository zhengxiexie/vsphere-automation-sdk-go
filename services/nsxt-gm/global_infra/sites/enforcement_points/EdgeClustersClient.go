...
type EdgeClustersClient interface {
	...
	Get(siteIdParam string, enforcementpointIdParam string, edgeClusterIdParam string) (nsx_global_policyModel.PolicyEdgeCluster, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List(siteIdParam string, enforcementpointIdParam string, cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_global_policyModel.PolicyEdgeClusterListResult, error)
}
...
func NewEdgeClustersClient(connector vapiProtocolClient_.Connector) *edgeClustersClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.sites.enforcement_points.edge_clusters")
...
func (eIface *edgeClustersClient) Get(siteIdParam string, enforcementpointIdParam string, edgeClusterIdParam string) (nsx_global_policyModel.PolicyEdgeCluster, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.PolicyEdgeCluster
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.PolicyEdgeCluster), nil
	} else {
...
func (eIface *edgeClustersClient) List(siteIdParam string, enforcementpointIdParam string, cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_global_policyModel.PolicyEdgeClusterListResult, error) {
	typeConverter := eIface.connector.TypeConverter()
	executionContext := eIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.PolicyEdgeClusterListResult
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.PolicyEdgeClusterListResult), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), eIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
			return emptyOutput, vapiBindings_.VAPIerrorsToError(errorInError)
