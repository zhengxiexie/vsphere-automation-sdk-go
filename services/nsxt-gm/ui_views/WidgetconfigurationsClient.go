...
type WidgetconfigurationsClient interface {
	...
	// The parameter must contain all the properties defined in nsx_global_policyModel.WidgetConfiguration.
	// @return com.vmware.nsx_global_policy.model.WidgetConfiguration
	// The return value will contain all the properties defined in nsx_global_policyModel.WidgetConfiguration.
	//
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Get(viewIdParam string, containerParam *string, widgetIdsParam *string) (nsx_global_policyModel.WidgetConfigurationList, error)
...
func NewWidgetconfigurationsClient(connector vapiProtocolClient_.Connector) *widgetconfigurationsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.ui_views.widgetconfigurations")
...
func (wIface *widgetconfigurationsClient) Get(viewIdParam string, containerParam *string, widgetIdsParam *string) (nsx_global_policyModel.WidgetConfigurationList, error) {
	typeConverter := wIface.connector.TypeConverter()
	executionContext := wIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.WidgetConfigurationList
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.WidgetConfigurationList), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), wIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
			return emptyOutput, vapiBindings_.VAPIerrorsToError(errorInError)
