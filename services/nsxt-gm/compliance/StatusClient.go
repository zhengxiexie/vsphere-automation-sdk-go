...
type StatusClient interface {
	...
	Get() (nsx_global_policyModel.PolicyComplianceStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.compliance.status")
...
func (sIface *statusClient) Get() (nsx_global_policyModel.PolicyComplianceStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.PolicyComplianceStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.PolicyComplianceStatus), nil
	} else {
