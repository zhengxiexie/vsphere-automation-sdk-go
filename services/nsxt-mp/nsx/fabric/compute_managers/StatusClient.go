...
type StatusClient interface {
	...
	Get(computeManagerIdParam string) (nsxModel.ComputeManagerStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.fabric.compute_managers.status")
...
func (sIface *statusClient) Get(computeManagerIdParam string) (nsxModel.ComputeManagerStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.ComputeManagerStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.ComputeManagerStatus), nil
	} else {
