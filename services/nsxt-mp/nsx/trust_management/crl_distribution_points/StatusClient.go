...
type StatusClient interface {
	...
	Get(crlDistributionPointIdParam string) (nsxModel.CrlDistributionPointStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.trust_management.crl_distribution_points.status")
...
func (sIface *statusClient) Get(crlDistributionPointIdParam string) (nsxModel.CrlDistributionPointStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.CrlDistributionPointStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.CrlDistributionPointStatus), nil
	} else {
