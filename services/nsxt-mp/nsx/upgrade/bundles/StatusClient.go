...
type StatusClient interface {
	...
	// @param operationParam Target operation (optional)
	// @return com.vmware.nsx.model.UpgradeBundleStatus
	//
	...
	Get(bundleIdParam string, operationParam *string) (nsxModel.UpgradeBundleStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.upgrade.bundles.status")
...
func (sIface *statusClient) Get(bundleIdParam string, operationParam *string) (nsxModel.UpgradeBundleStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.UpgradeBundleStatus
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.upgrade.bundles.status", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.UpgradeBundleStatus
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.UpgradeBundleStatus), nil
	} else {
