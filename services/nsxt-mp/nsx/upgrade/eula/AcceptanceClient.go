...
type AcceptanceClient interface {
	...
	Get() (nsxModel.EULAAcceptance, error)
}
...
func NewAcceptanceClient(connector vapiProtocolClient_.Connector) *acceptanceClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.upgrade.eula.acceptance")
...
func (aIface *acceptanceClient) Get() (nsxModel.EULAAcceptance, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.EULAAcceptance
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.EULAAcceptance), nil
	} else {
