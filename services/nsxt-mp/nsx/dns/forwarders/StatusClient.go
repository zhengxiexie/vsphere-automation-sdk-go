...
type StatusClient interface {
	...
	Get(forwarderIdParam string) (nsxModel.DnsForwarderStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.dns.forwarders.status")
...
func (sIface *statusClient) Get(forwarderIdParam string) (nsxModel.DnsForwarderStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.DnsForwarderStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.DnsForwarderStatus), nil
	} else {
