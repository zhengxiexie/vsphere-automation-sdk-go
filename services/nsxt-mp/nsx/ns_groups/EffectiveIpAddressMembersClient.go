...
type EffectiveIpAddressMembersClient interface {
	...
	List(nsGroupIdParam string, cursorParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsxModel.EffectiveIPAddressMemberListResult, error)
}
...
func NewEffectiveIpAddressMembersClient(connector vapiProtocolClient_.Connector) *effectiveIpAddressMembersClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.ns_groups.effective_ip_address_members")
...
func (eIface *effectiveIpAddressMembersClient) GetErrorBindingType(errorName string) vapiBindings_.BindingType {
...
func (eIface *effectiveIpAddressMembersClient) List(nsGroupIdParam string, cursorParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsxModel.EffectiveIPAddressMemberListResult, error) {
	...
	sv := vapiBindings_.NewStructValueBuilder(effectiveIpAddressMembersListInputType(), typeConverter)
	...
	if inputError != nil {
		var emptyOutput nsxModel.EffectiveIPAddressMemberListResult
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.EffectiveIPAddressMemberListResult), nil
	} else {
