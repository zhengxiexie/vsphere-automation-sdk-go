...
type StateClient interface {
	...
	Get(sectionIdParam string, barrierIdParam *int64, requestIdParam *string) (nsxModel.FirewallSectionState, error)
}
...
func NewStateClient(connector vapiProtocolClient_.Connector) *stateClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.firewall.sections.state")
...
func (sIface *stateClient) Get(sectionIdParam string, barrierIdParam *int64, requestIdParam *string) (nsxModel.FirewallSectionState, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.FirewallSectionState
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.FirewallSectionState), nil
	} else {
