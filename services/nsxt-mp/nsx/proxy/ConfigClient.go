...
type ConfigClient interface {
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(proxyParam nsxModel.Proxy) (nsxModel.Proxy, error)
}
...
func NewConfigClient(connector vapiProtocolClient_.Connector) *configClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.proxy.config")
...
func (cIface *configClient) Get() (nsxModel.Proxy, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.Proxy
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.Proxy), nil
	} else {
...
func (cIface *configClient) Update(proxyParam nsxModel.Proxy) (nsxModel.Proxy, error) {
	typeConverter := cIface.connector.TypeConverter()
	executionContext := cIface.connector.NewExecutionContext()
