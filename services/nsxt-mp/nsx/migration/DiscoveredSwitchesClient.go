...
type DiscoveredSwitchesClient interface {
	...
	List(cursorParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsxModel.MigrationSwitchListResult, error)
}
...
func NewDiscoveredSwitchesClient(connector vapiProtocolClient_.Connector) *discoveredSwitchesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.migration.discovered_switches")
...
func (dIface *discoveredSwitchesClient) GetErrorBindingType(errorName string) vapiBindings_.BindingType {
...
func (dIface *discoveredSwitchesClient) List(cursorParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsxModel.MigrationSwitchListResult, error) {
	...
	sv := vapiBindings_.NewStructValueBuilder(discoveredSwitchesListInputType(), typeConverter)
	...
	if inputError != nil {
		var emptyOutput nsxModel.MigrationSwitchListResult
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.MigrationSwitchListResult), nil
	} else {
