...
type NodesSummaryClient interface {
	...
	Get() (nsxModel.NodeSummaryList, error)
}
...
func NewNodesSummaryClient(connector vapiProtocolClient_.Connector) *nodesSummaryClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.migration.nodes_summary")
...
func (nIface *nodesSummaryClient) GetErrorBindingType(errorName string) vapiBindings_.BindingType {
...
func (nIface *nodesSummaryClient) Get() (nsxModel.NodeSummaryList, error) {
	...
	sv := vapiBindings_.NewStructValueBuilder(nodesSummaryGetInputType(), typeConverter)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsxModel.NodeSummaryList
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.NodeSummaryList), nil
	} else {
