...
type SearchClient interface {
	...
	Create(vcenterIdParam string, dvpgIdAndSegmentPathPairParam nsxModel.DvpgIdAndSegmentPathPair) (nsxModel.WorkflowListResult, error)
}
...
func NewSearchClient(connector vapiProtocolClient_.Connector) *searchClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.migration.overlay_adoption.vcenters.workflows.dvpg_segment_pair.search")
...
func (sIface *searchClient) Create(vcenterIdParam string, dvpgIdAndSegmentPathPairParam nsxModel.DvpgIdAndSegmentPathPair) (nsxModel.WorkflowListResult, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.WorkflowListResult
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.migration.overlay_adoption.vcenters.workflows.dvpg_segment_pair.search", "create", inputDataValue, executionContext)
	var emptyOutput nsxModel.WorkflowListResult
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.WorkflowListResult), nil
	} else {
