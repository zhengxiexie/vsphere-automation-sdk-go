...
type SearchClient interface {
	...
	// The return value will contain all the properties defined in nsxModel.WorkflowSpec.
	//
	...
	Create(vcenterIdParam string, workflowRequestParametersParam nsxModel.WorkflowRequestParameters) (*vapiData_.StructValue, error)
}
...
func NewSearchClient(connector vapiProtocolClient_.Connector) *searchClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.migration.overlay_adoption.vcenters.workflow.spec.search")
...
func (sIface *searchClient) Create(vcenterIdParam string, workflowRequestParametersParam nsxModel.WorkflowRequestParameters) (*vapiData_.StructValue, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.migration.overlay_adoption.vcenters.workflow.spec.search", "create", inputDataValue, executionContext)
	var emptyOutput *vapiData_.StructValue
