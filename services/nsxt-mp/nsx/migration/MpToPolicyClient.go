...
type MpToPolicyClient interface {
	...
	Create(mpMigrationDataParam nsxModel.MpMigrationData) error
}
...
func NewMpToPolicyClient(connector vapiProtocolClient_.Connector) *mpToPolicyClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.migration.mp_to_policy")
...
func (mIface *mpToPolicyClient) GetErrorBindingType(errorName string) vapiBindings_.BindingType {
...
func (mIface *mpToPolicyClient) Create(mpMigrationDataParam nsxModel.MpMigrationData) error {
	...
	sv := vapiBindings_.NewStructValueBuilder(mpToPolicyCreateInputType(), typeConverter)
