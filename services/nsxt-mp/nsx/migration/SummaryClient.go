...
type SummaryClient interface {
	...
	Get() (nsxModel.MigrationSummary, error)
}
...
func NewSummaryClient(connector vapiProtocolClient_.Connector) *summaryClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.migration.summary")
...
func (sIface *summaryClient) Get() (nsxModel.MigrationSummary, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.MigrationSummary
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.MigrationSummary), nil
	} else {
