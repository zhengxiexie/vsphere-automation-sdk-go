...
type ConfigClient interface {
	...
	Get() (nsxModel.BackupConfiguration, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(backupConfigurationParam nsxModel.BackupConfiguration, frameTypeParam *string, siteIdParam *string) (nsxModel.BackupConfiguration, error)
}
...
func NewConfigClient(connector vapiProtocolClient_.Connector) *configClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.backups.config")
...
func (cIface *configClient) Get() (nsxModel.BackupConfiguration, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.BackupConfiguration
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.BackupConfiguration), nil
	} else {
...
func (cIface *configClient) Update(backupConfigurationParam nsxModel.BackupConfiguration, frameTypeParam *string, siteIdParam *string) (nsxModel.BackupConfiguration, error) {
	typeConverter := cIface.connector.TypeConverter()
	executionContext := cIface.connector.NewExecutionContext()
