...
type StatusClient interface {
	...
	Get() (nsxModel.CurrentBackupOperationStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.backups.status")
...
func (sIface *statusClient) Get() (nsxModel.CurrentBackupOperationStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.CurrentBackupOperationStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.CurrentBackupOperationStatus), nil
	} else {
