...
type HistoryClient interface {
	...
	Get() (nsxModel.BackupOperationHistory, error)
}
...
func NewHistoryClient(connector vapiProtocolClient_.Connector) *historyClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.backups.history")
...
func (hIface *historyClient) Get() (nsxModel.BackupOperationHistory, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.BackupOperationHistory
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.BackupOperationHistory), nil
	} else {
