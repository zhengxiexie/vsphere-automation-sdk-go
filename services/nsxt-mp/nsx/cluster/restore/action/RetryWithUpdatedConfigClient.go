...
type Retry_with_updated_configClient interface {
	...
	Create() (nsxModel.ClusterRestoreStatus, error)
}
...
func NewRetry_with_updated_configClient(connector vapiProtocolClient_.Connector) *retry_with_updated_configClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.restore.action.retry_with_updated_config")
...
func (rIface *retry_with_updated_configClient) Create() (nsxModel.ClusterRestoreStatus, error) {
	...
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.restore.action.retry_with_updated_config", "create", inputDataValue, executionContext)
	var emptyOutput nsxModel.ClusterRestoreStatus
