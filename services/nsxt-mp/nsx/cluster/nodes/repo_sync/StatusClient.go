...
type StatusClient interface {
	...
	Get(nodeIdParam string) (nsxModel.RepoSyncStatusReport, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.nodes.repo_sync.status")
...
func (sIface *statusClient) Get(nodeIdParam string) (nsxModel.RepoSyncStatusReport, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.RepoSyncStatusReport
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.RepoSyncStatusReport), nil
	} else {
