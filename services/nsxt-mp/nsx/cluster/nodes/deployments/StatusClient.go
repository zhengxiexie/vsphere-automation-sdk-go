...
type StatusClient interface {
	...
	Get(nodeIdParam string) (nsxModel.ClusterNodeVMDeploymentStatusReport, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.nodes.deployments.status")
...
func (sIface *statusClient) Get(nodeIdParam string) (nsxModel.ClusterNodeVMDeploymentStatusReport, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.ClusterNodeVMDeploymentStatusReport
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.ClusterNodeVMDeploymentStatusReport), nil
	} else {
