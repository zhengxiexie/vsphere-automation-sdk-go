...
type StatusClient interface {
	...
	Get(nodeIdParam string, sourceParam *string) (nsxModel.ClusterNodeStatus, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Getall() (nsxModel.ClustersAggregateInfo, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.nodes.status")
...
func (sIface *statusClient) Get(nodeIdParam string, sourceParam *string) (nsxModel.ClusterNodeStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.ClusterNodeStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.ClusterNodeStatus), nil
	} else {
...
func (sIface *statusClient) Getall() (nsxModel.ClustersAggregateInfo, error) {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsxModel.ClustersAggregateInfo
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.ClustersAggregateInfo), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
			return emptyOutput, vapiBindings_.VAPIerrorsToError(errorInError)
