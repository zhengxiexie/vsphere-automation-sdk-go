...
type StatusClient interface {
	...
	Get() (nsxModel.ClusterStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.status")
...
func (sIface *statusClient) Get() (nsxModel.ClusterStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.ClusterStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.ClusterStatus), nil
	} else {
