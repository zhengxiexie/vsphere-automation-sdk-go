...
type RulesClient interface {
	...
	Get(logicalRouterIdParam string) (nsxModel.RedistributionRuleList, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(logicalRouterIdParam string, redistributionRuleListParam nsxModel.RedistributionRuleList) (nsxModel.RedistributionRuleList, error)
}
...
func NewRulesClient(connector vapiProtocolClient_.Connector) *rulesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.logical_routers.routing.redistribution.rules")
...
func (rIface *rulesClient) Get(logicalRouterIdParam string) (nsxModel.RedistributionRuleList, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.RedistributionRuleList
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.RedistributionRuleList), nil
	} else {
...
func (rIface *rulesClient) Update(logicalRouterIdParam string, redistributionRuleListParam nsxModel.RedistributionRuleList) (nsxModel.RedistributionRuleList, error) {
	typeConverter := rIface.connector.TypeConverter()
	executionContext := rIface.connector.NewExecutionContext()
