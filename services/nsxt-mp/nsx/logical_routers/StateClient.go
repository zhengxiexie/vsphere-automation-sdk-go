...
type StateClient interface {
	...
	Get(logicalRouterIdParam string, barrierIdParam *int64, requestIdParam *string) (nsxModel.LogicalRouterState, error)
}
...
func NewStateClient(connector vapiProtocolClient_.Connector) *stateClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.logical_routers.state")
...
func (sIface *stateClient) Get(logicalRouterIdParam string, barrierIdParam *int64, requestIdParam *string) (nsxModel.LogicalRouterState, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.LogicalRouterState
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.LogicalRouterState), nil
	} else {
