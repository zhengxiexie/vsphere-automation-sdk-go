...
type StateClient interface {
	...
	Get(serverIdParam string, bindingIdParam string, barrierIdParam *int64, requestIdParam *string) (nsxModel.ConfigurationState, error)
}
...
func NewStateClient(connector vapiProtocolClient_.Connector) *stateClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.dhcp.servers.static_bindings.state")
...
func (sIface *stateClient) Get(serverIdParam string, bindingIdParam string, barrierIdParam *int64, requestIdParam *string) (nsxModel.ConfigurationState, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.ConfigurationState
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.ConfigurationState), nil
	} else {
