...
type StatisticsClient interface {
	...
	Get(serverIdParam string) (nsxModel.DhcpStatistics, error)
}
...
func NewStatisticsClient(connector vapiProtocolClient_.Connector) *statisticsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.dhcp.servers.statistics")
...
func (sIface *statisticsClient) Get(serverIdParam string) (nsxModel.DhcpStatistics, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.DhcpStatistics
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.DhcpStatistics), nil
	} else {
