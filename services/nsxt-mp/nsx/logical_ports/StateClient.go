...
type StateClient interface {
	...
	Get(lportIdParam string) (nsxModel.LogicalPortState, error)
}
...
func NewStateClient(connector vapiProtocolClient_.Connector) *stateClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.logical_ports.state")
...
func (sIface *stateClient) Get(lportIdParam string) (nsxModel.LogicalPortState, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.LogicalPortState
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.LogicalPortState), nil
	} else {
