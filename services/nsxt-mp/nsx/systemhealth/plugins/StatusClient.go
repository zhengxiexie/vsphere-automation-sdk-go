...
type StatusClient interface {
	...
	List(nodeIdParam string) (nsxModel.PluginStatusList, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.systemhealth.plugins.status")
...
func (sIface *statusClient) List(nodeIdParam string) (nsxModel.PluginStatusList, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.PluginStatusList
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.PluginStatusList), nil
	} else {
