...
type StatusClient interface {
	...
	Get(nodeIdParam string) (nsxModel.TnHyperbusStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.systemhealth.transport_nodes.container.hyperbus.status")
...
func (sIface *statusClient) Get(nodeIdParam string) (nsxModel.TnHyperbusStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.TnHyperbusStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.TnHyperbusStatus), nil
	} else {
