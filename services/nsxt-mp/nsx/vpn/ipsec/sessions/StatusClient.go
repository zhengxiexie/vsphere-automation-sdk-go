...
type StatusClient interface {
	...
	Get(sessionIdParam string, sourceParam *string) (nsxModel.IPSecVPNSessionStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.vpn.ipsec.sessions.status")
...
func (sIface *statusClient) Get(sessionIdParam string, sourceParam *string) (nsxModel.IPSecVPNSessionStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.IPSecVPNSessionStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.IPSecVPNSessionStatus), nil
	} else {
