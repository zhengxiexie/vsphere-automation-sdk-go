...
type StatusClient interface {
	...
	Get(sessionIdParam string, sourceParam *string) (nsxModel.L2VPNSessionStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.vpn.l2vpn.sessions.status")
...
func (sIface *statusClient) Get(sessionIdParam string, sourceParam *string) (nsxModel.L2VPNSessionStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.L2VPNSessionStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.L2VPNSessionStatus), nil
	} else {
