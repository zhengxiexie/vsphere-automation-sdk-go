...
type SummaryClient interface {
	...
	Get(sourceParam *string) (nsxModel.L2VPNSessionSummary, error)
}
...
func NewSummaryClient(connector vapiProtocolClient_.Connector) *summaryClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.vpn.l2vpn.sessions.summary")
...
func (sIface *summaryClient) Get(sourceParam *string) (nsxModel.L2VPNSessionSummary, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.L2VPNSessionSummary
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.L2VPNSessionSummary), nil
	} else {
