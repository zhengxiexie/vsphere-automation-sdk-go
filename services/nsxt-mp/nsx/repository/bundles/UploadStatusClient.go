...
type UploadStatusClient interface {
	...
	Get(bundleIdParam string, productParam string) (nsxModel.BundleUploadStatus, error)
}
...
func NewUploadStatusClient(connector vapiProtocolClient_.Connector) *uploadStatusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.repository.bundles.upload_status")
...
func (uIface *uploadStatusClient) Get(bundleIdParam string, productParam string) (nsxModel.BundleUploadStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.BundleUploadStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.BundleUploadStatus), nil
	} else {
