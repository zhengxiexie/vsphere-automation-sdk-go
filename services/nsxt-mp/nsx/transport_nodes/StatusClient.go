...
type StatusClient interface {
	...
	Get(nodeTypeParam *string) (nsxModel.HeatMapTransportZoneStatus, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Get0(nodeIdParam string, sourceParam *string) (nsxModel.TransportNodeStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.transport_nodes.status")
...
func (sIface *statusClient) Get(nodeTypeParam *string) (nsxModel.HeatMapTransportZoneStatus, error) {
...
func (sIface *statusClient) Get0(nodeIdParam string, sourceParam *string) (nsxModel.TransportNodeStatus, error) {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsxModel.TransportNodeStatus
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.TransportNodeStatus), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
			return emptyOutput, vapiBindings_.VAPIerrorsToError(errorInError)
