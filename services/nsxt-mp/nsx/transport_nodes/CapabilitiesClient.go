...
type CapabilitiesClient interface {
	...
	Get(transportNodeIdParam string) (nsxModel.NodeCapabilitiesResult, error)
}
...
func NewCapabilitiesClient(connector vapiProtocolClient_.Connector) *capabilitiesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.transport_nodes.capabilities")
...
func (cIface *capabilitiesClient) GetErrorBindingType(errorName string) vapiBindings_.BindingType {
...
func (cIface *capabilitiesClient) Get(transportNodeIdParam string) (nsxModel.NodeCapabilitiesResult, error) {
	...
	sv := vapiBindings_.NewStructValueBuilder(capabilitiesGetInputType(), typeConverter)
	sv.AddStructField("TransportNodeId", transportNodeIdParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsxModel.NodeCapabilitiesResult
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.NodeCapabilitiesResult), nil
	} else {
