...
type SelfClient interface {
	...
	Get() (nsxModel.LocalSiteConfiguration, error)
}
...
func NewSelfClient(connector vapiProtocolClient_.Connector) *selfClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.sites.self")
...
func (sIface *selfClient) GetErrorBindingType(errorName string) vapiBindings_.BindingType {
...
func (sIface *selfClient) Get() (nsxModel.LocalSiteConfiguration, error) {
	...
	sv := vapiBindings_.NewStructValueBuilder(selfGetInputType(), typeConverter)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsxModel.LocalSiteConfiguration
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.LocalSiteConfiguration), nil
	} else {
