...
type StatusClient interface {
	...
	Get() (nsxModel.FederationStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.sites.status")
...
func (sIface *statusClient) Get() (nsxModel.FederationStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.FederationStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.FederationStatus), nil
	} else {
