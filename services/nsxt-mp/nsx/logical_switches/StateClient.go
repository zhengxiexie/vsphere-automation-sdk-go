...
type StateClient interface {
	...
	Get(lswitchIdParam string) (nsxModel.LogicalSwitchState, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List(statusParam *string) (nsxModel.LogicalSwitchStateListResult, error)
}
...
func NewStateClient(connector vapiProtocolClient_.Connector) *stateClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.logical_switches.state")
...
func (sIface *stateClient) Get(lswitchIdParam string) (nsxModel.LogicalSwitchState, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.LogicalSwitchState
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.LogicalSwitchState), nil
	} else {
...
func (sIface *stateClient) List(statusParam *string) (nsxModel.LogicalSwitchStateListResult, error) {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsxModel.LogicalSwitchStateListResult
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.LogicalSwitchStateListResult), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
			return emptyOutput, vapiBindings_.VAPIerrorsToError(errorInError)
