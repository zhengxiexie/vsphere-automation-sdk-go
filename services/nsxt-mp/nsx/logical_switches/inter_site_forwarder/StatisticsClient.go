...
type StatisticsClient interface {
	...
	Get(logicalSwitchIdParam string) (nsxModel.L2ForwarderStatistics, error)
}
...
func NewStatisticsClient(connector vapiProtocolClient_.Connector) *statisticsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.logical_switches.inter_site_forwarder.statistics")
...
func (sIface *statisticsClient) Get(logicalSwitchIdParam string) (nsxModel.L2ForwarderStatistics, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.L2ForwarderStatistics
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.L2ForwarderStatistics), nil
	} else {
