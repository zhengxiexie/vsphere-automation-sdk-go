...
type StatusClient interface {
	...
	Get(logicalSwitchIdParam string, sourceParam *string) (nsxModel.L2ForwarderStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.logical_switches.inter_site_forwarder.status")
...
func (sIface *statusClient) Get(logicalSwitchIdParam string, sourceParam *string) (nsxModel.L2ForwarderStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.L2ForwarderStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.L2ForwarderStatus), nil
	} else {
