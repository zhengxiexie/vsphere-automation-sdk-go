...
type StateClient interface {
	...
	Get(transportNodeCollectionIdParam string) (nsxModel.TransportNodeCollectionState, error)
}
...
func NewStateClient(connector vapiProtocolClient_.Connector) *stateClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.transport_node_collections.state")
...
func (sIface *stateClient) Get(transportNodeCollectionIdParam string) (nsxModel.TransportNodeCollectionState, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.TransportNodeCollectionState
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.TransportNodeCollectionState), nil
	} else {
