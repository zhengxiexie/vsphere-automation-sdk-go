...
type UserInfoClient interface {
	...
	Get(provideFlatListingParam *bool, rootPathParam *string) (nsxModel.UserInfo, error)
}
...
func NewUserInfoClient(connector vapiProtocolClient_.Connector) *userInfoClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.aaa.user_info")
...
func (uIface *userInfoClient) Get(provideFlatListingParam *bool, rootPathParam *string) (nsxModel.UserInfo, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.UserInfo
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.UserInfo), nil
	} else {
