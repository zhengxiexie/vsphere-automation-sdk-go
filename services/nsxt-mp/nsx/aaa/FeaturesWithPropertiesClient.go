...
type FeaturesWithPropertiesClient interface {
	...
	List() (nsxModel.FeaturePermissionListResult, error)
}
...
func NewFeaturesWithPropertiesClient(connector vapiProtocolClient_.Connector) *featuresWithPropertiesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.aaa.features_with_properties")
...
func (fIface *featuresWithPropertiesClient) List() (nsxModel.FeaturePermissionListResult, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.FeaturePermissionListResult
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.FeaturePermissionListResult), nil
	} else {
