...
type StatusClient interface {
	...
	Get() (nsxModel.NodeAuthProviderVidmStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.node.aaa.providers.vidm.status")
...
func (sIface *statusClient) Get() (nsxModel.NodeAuthProviderVidmStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.NodeAuthProviderVidmStatus
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.NodeAuthProviderVidmStatus), nil
	} else {
