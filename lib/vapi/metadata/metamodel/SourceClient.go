...
type SourceClient interface {
	...
	// @throws InvalidArgument if the file specified in vapiMetadata_.SourceCreateSpec#filepath is not a valid JSON file or if the format of the metamodel metadata in the JSON file is invalid.
	// @throws InvalidArgument if the URI specified in vapiMetadata_.SourceCreateSpec#address is unreachable or if there is a transport protocol or message protocol mismatch between the client and the server or if the remote server do not have interfaces present in com.vmware.vapi.metadata.metamodel package.
...
func NewSourceClient(connector vapiProtocolClient_.Connector) *sourceClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.vapi.metadata.metamodel.source")
